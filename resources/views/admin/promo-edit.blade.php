        ...
        <form method="POST" action="{{ route('admin.promo.update', $promo->id) }}" enctype="multipart/form-data">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" id="kode_promo" name="kode_promo" value="{{ old('kode_promo', $promo->kode_promo) }}" placeholder="Contoh: PROMO50" required>
                </div>
                <div class="form-group">
                    <label for="nama_promo">Nama Promo <span style="color: red">*</span></label>
                    <input type="text" id="nama_promo" name="nama_promo" value="{{ old('nama_promo', $promo->nama_promo) }}" placeholder="Contoh: Diskon 50%" required>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input type="number" id="nilai_diskon" name="nilai_diskon" value="{{ old('nilai_diskon', $promo->nilai_diskon) }}" min="0" step="0.01" placeholder="Contoh: 50" required>
                </div>
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="number" id="maksimal_diskon" name="maksimal_diskon" value="{{ old('maksimal_diskon', $promo->maksimal_diskon) }}" min="0" placeholder="Contoh: 50000">
                </div>
                <div class="form-group">
                    <label for="minimal_pembelian">Minimal Pembelian (Rp)</label>
                    <input type="number" id="minimal_pembelian" name="minimal_pembelian" value="{{ old('minimal_pembelian', $promo->minimal_pembelian) }}" min="0" placeholder="Contoh: 100000">
                </div>
                <div class="form-group">
                    <label for="min_tiket">Minimal Tiket</label>
                    <input type="number" id="min_tiket" name="min_tiket" value="{{ old('min_tiket', $promo->min_tiket) }}" min="0" placeholder="Contoh: 1">
                </div>
                <div class="form-group">
                    <label for="kuota">Kuota Penggunaan</label>
                    <input type="number" id="kuota" name="kuota" value="{{ old('kuota', $promo->kuota) }}" min="0" placeholder="Kosongkan jika unlimited">
                </div>
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <select id="kategori_promo" name="kategori_promo" required>
                        ...
                        <option value="umum" {{ old('kategori_promo', $promo->kategori_promo) == 'umum' ? 'selected' : '' }}>Umum</option>
                        <option value="keluarga" {{ old('kategori_promo', $promo->kategori_promo) == 'keluarga' ? 'selected' : '' }}>Keluarga</option>
                        <option value="membership" {{ old('kategori_promo', $promo->kategori_promo) == 'membership' ? 'selected' : '' }}>Membership</option>
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <select id="tipe_promo" name="tipe_promo" required>
                        ...
                        <option value="all" {{ old('tipe_promo', $promo->tipe_promo) == 'all' ? 'selected' : '' }}>All (Semua)</option>
                        <option value="shuttle" {{ old('tipe_promo', $promo->tipe_promo) == 'shuttle' ? 'selected' : '' }}>Shuttle</option>
                        <option value="paket" {{ old('tipe_promo', $promo->tipe_promo) == 'paket' ? 'selected' : '' }}>Paket</option>
                        <option value="sewa" {{ old('tipe_promo', $promo->tipe_promo) == 'sewa' ? 'selected' : '' }}>Sewa</option>
                    </select>
                </div>
                <div class="form-group">
                    <label for="tanggal_mulai">Tanggal Mulai <span style="color: red">*</span></label>
                    <input type="date" id="tanggal_mulai" name="tanggal_mulai" value="{{ old('tanggal_mulai', \Carbon\Carbon::parse($promo->tanggal_mulai)->format('Y-m-d')) }}" required>
                </div>
                <div class="form-group">
                    <label for="tanggal_berakhir">Tanggal Berakhir <span style="color: red">*</span></label>
                    <input type="date" id="tanggal_berakhir" name="tanggal_berakhir" value="{{ old('tanggal_berakhir', \Carbon\Carbon::parse($promo->tanggal_berakhir)->format('Y-m-d')) }}" required>
                </div>
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <textarea id="deskripsi" name="deskripsi" placeholder="Jelaskan detail promo ini..." required>{{ old('deskripsi', $promo->deskripsi) }}</textarea>
                </div>
                <div class="form-group">
                    <label for="pesan_error">Pesan Error</label>
                    <textarea id="pesan_error" name="pesan_error" placeholder="Pesan yang ditampilkan jika promo tidak bisa digunakan">{{ old('pesan_error', $promo->pesan_error) }}</textarea>
                </div>
            </div>
                ...
                <div class="form-group">
                    ...
                    @if($promo->gambar)
                        <div style="margin-top: 10px;">
                            <img src="{{ asset('storage/' . $promo->gambar) }}" alt="Gambar Promo" style="max-width: 200px; border-radius: 8px; border: 1px solid #ddd;">
                        </div>
                    @endif
                </div>
                <div class="form-group">
                    ...
                    <div style="display: flex; flex-direction: column; gap: 10px; margin-top: 8px;">
                        <label style="display: flex; align-items: center; gap: 8px; font-weight: normal;">
                            <input type="checkbox" id="khusus_member" name="khusus_member" value="1" {{ old('khusus_member', $promo->khusus_member) ? 'checked' : '' }}>
                            Khusus Member
                        </label>
                        <label style="display: flex; align-items: center; gap: 8px; font-weight: normal;">
                            <input type="checkbox" id="status" name="status" value="1" {{ old('status', $promo->status) ? 'checked' : '' }}>
                            Aktif
                        </label>
                    </div>
                </div>

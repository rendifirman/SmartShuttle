        ...
        <form method="POST" action="{{ route('admin.artikel.update', $artikel->id) }}" enctype="multipart/form-data">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" id="judul" name="judul" value="{{ old('judul', $artikel->judul) }}" placeholder="Masukkan judul artikel" required>
                </div>
                <div class="form-group">
                    ...
                    <select id="kategori" name="kategori" required>
                        ...
                        <option value="promo" {{ old('kategori', $artikel->kategori) == 'promo' ? 'selected' : '' }}>Promo</option>
                        <option value="layanan" {{ old('kategori', $artikel->kategori) == 'layanan' ? 'selected' : '' }}>Layanan</option>
                    </select>
                </div>
                <div class="form-group">
                    <label for="penulis">Penulis <span style="color: red">*</span></label>
                    <input type="text" id="penulis" name="penulis" value="{{ old('penulis', $artikel->penulis) }}" placeholder="Nama penulis" required>
                </div>
                <div class="form-group">
                    <label for="tanggal_publikasi">Tanggal Publikasi <span style="color: red">*</span></label>
                    <input type="date" id="tanggal_publikasi" name="tanggal_publikasi" value="{{ old('tanggal_publikasi', $artikel->tanggal_publikasi ? $artikel->tanggal_publikasi->format('Y-m-d') : '') }}" required>
                </div>
            </div>
                ...
                <div class="form-group">
                    ...
                    <div style="display: flex; flex-direction: column; gap: 10px; margin-top: 8px;">
                        <label style="display: flex; align-items: center; gap: 8px; font-weight: normal;">
                            <input type="checkbox" id="status" name="status" value="1" {{ old('status', $artikel->status) ? 'checked' : '' }}>

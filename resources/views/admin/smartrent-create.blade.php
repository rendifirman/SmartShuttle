...
<body>
    ...
    <div class="main-container">
        ...
        @if(session('success'))
            <script>
                // show alert box in Indonesian when data is saved
        ...
        <form action="{{ (isset($isEdit) && $isEdit) ? route('admin.smartrent.update', $smartRent->id) : route('admin.smartrent.store') }}" 
            ...
            <div class="form-section">
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" name="nama_pelanggan" placeholder="Masukkan nama lengkap" value="{{ (isset($smartRent) ? $smartRent->nama_pelanggan : old('nama_pelanggan')) }}" required>
                    </div>

                    <div class="form-group">
                        <label>No. Telepon <span class="required">*</span></label>
                        <input type="tel" name="telepon" placeholder="08xxxxxxxxxx" value="{{ (isset($smartRent) ? $smartRent->telepon : old('telepon')) }}" required>
                    </div>

                    <div class="form-group">
                        <label>Email</label>
                        <input type="email" name="email" placeholder="email@contoh.com" value="{{ (isset($smartRent) ? $smartRent->email : old('email')) }}">
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group full-width">
                        <label>Alamat</label>
                        <textarea name="alamat" rows="2" placeholder="Masukkan alamat lengkap">{{ (isset($smartRent) ? $smartRent->alamat : old('alamat')) }}</textarea>
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" name="no_identitas" placeholder="16 digit NIK / No. SIM" value="{{ (isset($smartRent) ? $smartRent->no_identitas : old('no_identitas')) }}">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="jenis_identitas">
                            ...
                            <option value="ktp" {{ (isset($smartRent) && $smartRent->jenis_identitas == 'ktp') || old('jenis_identitas') == 'ktp' ? 'selected' : '' }}>KTP</option>
                            <option value="sim" {{ (isset($smartRent) && $smartRent->jenis_identitas == 'sim') || old('jenis_identitas') == 'sim' ? 'selected' : '' }}>SIM</option>
                            <option value="paspor" {{ (isset($smartRent) && $smartRent->jenis_identitas == 'paspor') || old('jenis_identitas') == 'paspor' ? 'selected' : '' }}>Paspor</option>
                        </select>
                    </div>
                        ...
                        <select name="customer_id" class="form-control">
                            ...
                            @if(isset($customers) && $customers->count() > 0)
                                @foreach($customers as $customer)
                                    <option value="{{ $customer->id }}" {{ (isset($smartRent) && $smartRent->customer_id == $customer->id) || old('customer_id') == $customer->id ? 'selected' : '' }}>
            ...
            <div class="form-section">
                ...
                <div class="form-row two-column">
                    <div class="form-group">
                        ...
                        <input type="date" name="tanggal_mulai" id="tanggal_mulai" value="{{ (isset($smartRent) ? $smartRent->tanggal_mulai->format('Y-m-d') : old('tanggal_mulai', date('Y-m-d'))) }}" required>
                    </div>

                    <div class="form-group">
                        <label>Tanggal Selesai <span class="required">*</span></label>
                        <input type="date" name="tanggal_selesai" id="tanggal_selesai" value="{{ (isset($smartRent) ? $smartRent->tanggal_selesai->format('Y-m-d') : old('tanggal_selesai', date('Y-m-d', strtotime('+1 day')))) }}" required>
                    </div>
                </div>
                ...
                <div class="form-row two-column">
                    <div class="form-group">
                        ...
                        <input type="number" name="durasi" id="durasi" placeholder="Durasi sewa (otomatis dihitung)" value="{{ (isset($smartRent) ? $smartRent->durasi : old('durasi', 1)) }}" readonly required>
                    </div>

                    <div class="form-group">
                        <label>Jumlah Mobil <span class="required">*</span></label>
                        <input type="number" name="jumlah_mobil" id="jumlah_mobil" placeholder="Jumlah unit mobil" value="{{ (isset($smartRent) ? $smartRent->jumlah_mobil : old('jumlah_mobil', 1)) }}" min="1" max="10" required>
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <div class="combo-input-group">
                            <input type="text" id="kota-asal-input" placeholder="Cari atau pilih kota asal" value="{{ (isset($smartRent) ? $smartRent->kota_asal : old('kota_asal', 'Jakarta')) }}">
                            <button type="button" class="combo-dropdown-toggle" tabindex="-1">
                                <i class="fas fa-chevron-down"></i>
                            </button>
                            <div id="kota-asal-dropdown" class="combo-dropdown" style="display: none;">
                                <input type="text" placeholder="Cari..." style="margin-bottom: 8px;">
                                ...
                                <div class="combo-option" data-value="Semarang">Semarang</div>
                                <div class="combo-option" data-value="Medan">Medan</div>
                                <div class="combo-option" data-value="Makassar">Makassar</div>
                                <div class="combo-option" data-value="Palembang">Palembang</div>
                                <div class="combo-option" data-value="Batam">Batam</div>
                            </div>
                        </div>
                        <input type="hidden" name="kota_asal" id="kota_asal" value="{{ (isset($smartRent) ? $smartRent->kota_asal : old('kota_asal', 'Jakarta')) }}">
                    </div>

                    <div class="form-group">
                        <label>Kota Tujuan <span class="required">*</span></label>
                        <div class="combo-input-group">
                            <input type="text" id="kota-tujuan-input" placeholder="Cari atau pilih kota tujuan" value="{{ (isset($smartRent) ? $smartRent->kota_tujuan : old('kota_tujuan', 'Bandung')) }}">
                            <button type="button" class="combo-dropdown-toggle" tabindex="-1">
                                <i class="fas fa-chevron-down"></i>
                            </button>
                            <div id="kota-tujuan-dropdown" class="combo-dropdown" style="display: none;">
                                <input type="text" placeholder="Cari..." style="margin-bottom: 8px;">
                        ...
                        <input type="hidden" name="kota_tujuan" id="kota_tujuan" value="{{ (isset($smartRent) ? $smartRent->kota_tujuan : old('kota_tujuan', 'Bandung')) }}">
                    </div>
                </div>
            </div>
            ...
            <div class="form-section">
                ...
                <div class="form-row" style="margin-bottom: 20px;">
                    ...
                    <div class="form-group">
                        ...
                        <select name="layanan" id="layanan" required>
                            ...
                            <option value="lepas_kunci" {{ (isset($smartRent) && $smartRent->layanan == 'lepas_kunci') || old('layanan') == 'lepas_kunci' ? 'selected' : '' }}>Lepas Kunci</option>
                            <option value="dengan_sopir" {{ (isset($smartRent) && $smartRent->layanan == 'dengan_sopir') || old('layanan') == 'dengan_sopir' ? 'selected' : '' }}>Dengan Sopir</option>
                        </select>
                    </div>
                </div>

                <!-- Dropdown: quick select for Armada (keeps cards in sync) -->
                <div class="form-row" style="margin-bottom: 16px;">
                    <div class="form-group">
                        <label>Pilih Armada (Cepat)</label>
                        <select id="armada-select" style="width:100%; padding:8px;">
                            <option value="">-- Pilih Armada --</option>
                            @foreach($armadaList as $armada)
                                <option value="{{ $armada->id }}" data-harga="{{ $armada->harga_dasar }}"
                                    {{ (isset($smartRent) && $smartRent->armada_id == $armada->id) || old('armada_id') == $armada->id ? 'selected' : '' }}>
                                    {{ $armada->nama }} — Rp {{ number_format($armada->harga_dasar, 0, ',', '.') }} /hari
                                </option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div id="armada-list">
                    ...
                    <div class="armada-container" 
                        ...
                        <input type="radio" name="armada_id" value="{{ $armada->id }}" class="armada-radio-hidden" 
                               {{ (isset($smartRent) && $smartRent->armada_id == $armada->id) || old('armada_id') == $armada->id ? 'checked' : '' }}>
                        ...
                        <div class="armada-header">
                            <div class="armada-left">
                                ...
                                <div class="armada-badges">
                                    <span class="badge badge-primary">
                                        ...
                                        {{ $armada->tipe }} - {{ $armada->kapasitas }} Seat
                                    </span>
                                    <span class="badge badge-success">
                                        <i class="fas fa-check-circle"></i> {{ $armada->status == 'aktif' ? 'Tersedia' : 'Tidak Tersedia' }}
                                    </span>
                                </div>
                                ...
                                <div class="armada-route">
                                    ...
                                    <div>
                                        <div class="route-time">Selesai</div>
                                ...
                                <div class="armada-facilities">
                                    @if($armada->fasilitas && is_array($armada->fasilitas))
                                        @foreach($armada->fasilitas as $fasilitas)
                                        <span class="facility-badge">
                                            @if(in_array($fasilitas, ['AC', 'AC Double']))
                                                <i class="fas fa-snowflake"></i>
                                            @elseif($fasilitas == 'Audio')
                                                <i class="fas fa-music"></i>
                                            @elseif(in_array($fasilitas, ['Charger', 'USB Charger']))
                            ...
                            <div class="armada-right">
                                <div class="price-section">
                                    <div class="price-amount">Rp {{ number_format($armada->harga_dasar, 0, ',', '.') }} <span class="price-unit">/hari</span></div>
                                    <div class="price-total" id="total-armada-{{ $armada->id }}">Total: <strong>Rp {{ number_format($armada->harga_dasar, 0, ',', '.') }}</strong></div>
                                </div>
                    ...
                    <div style="padding: 20px; text-align: center; color: #999;">
                        <p><i class="fas fa-info-circle"></i> Tidak ada armada aktif yang tersedia saat ini.</p>
                    </div>
                    @endforelse
                </div>
            </div>
            ...
            <div class="form-section">
                ...
                <div class="payment-grid">
                    ...
                    <div class="payment-box">
                        ...
                        <div class="payment-method-group">
                            <label class="payment-method {{ (isset($smartRent) && $smartRent->metode_pembayaran == 'BCA VA') || old('metode_pembayaran') == 'BCA VA' ? 'selected' : '' }}">
                                <input type="radio" name="metode_pembayaran" value="BCA VA" {{ (isset($smartRent) && $smartRent->metode_pembayaran == 'BCA VA') || old('metode_pembayaran') == 'BCA VA' ? 'checked' : '' }} required>
                            ...
                            <label class="payment-method {{ (isset($smartRent) && $smartRent->metode_pembayaran == 'Mandiri VA') || old('metode_pembayaran') == 'Mandiri VA' ? 'selected' : '' }}">
                                <input type="radio" name="metode_pembayaran" value="Mandiri VA" {{ (isset($smartRent) && $smartRent->metode_pembayaran == 'Mandiri VA') || old('metode_pembayaran') == 'Mandiri VA' ? 'checked' : '' }}>
                                <span class="payment-method-icon">
                                    <i class="fas fa-university"></i>
                                </span>
                                <span class="payment-method-info">
                            ...
                            <label class="payment-method {{ (isset($smartRent) && $smartRent->metode_pembayaran == 'QRIS') || old('metode_pembayaran') == 'QRIS' ? 'selected' : '' }}">
                                <input type="radio" name="metode_pembayaran" value="QRIS" {{ (isset($smartRent) && $smartRent->metode_pembayaran == 'QRIS') || old('metode_pembayaran') == 'QRIS' ? 'checked' : '' }}>
                                <span class="payment-method-icon">
                            ...
                            <label class="payment-method {{ (isset($smartRent) && $smartRent->metode_pembayaran == 'Transfer Bank') || old('metode_pembayaran') == 'Transfer Bank' ? 'selected' : '' }}">
                                <input type="radio" name="metode_pembayaran" value="Transfer Bank" {{ (isset($smartRent) && $smartRent->metode_pembayaran == 'Transfer Bank') || old('metode_pembayaran') == 'Transfer Bank' ? 'checked' : '' }}>
                                <span class="payment-method-icon">
                            ...
                            <label class="payment-method {{ (isset($smartRent) && $smartRent->metode_pembayaran == 'Cash') || old('metode_pembayaran') == 'Cash' ? 'selected' : '' }}">
                                <input type="radio" name="metode_pembayaran" value="Cash" {{ (isset($smartRent) && $smartRent->metode_pembayaran == 'Cash') || old('metode_pembayaran') == 'Cash' ? 'checked' : '' }}>
                                <span class="payment-method-icon">
                ...
                <div class="catatan-section">
                    ...
                    <textarea name="catatan" id="catatan" placeholder="Masukkan catatan tambahan (opsional)">{{ (isset($smartRent) ? $smartRent->catatan : old('catatan')) }}</textarea>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <select name="status" required>
                            ...
                            <option value="pending" {{ $smartRent->status == 'pending' ? 'selected' : '' }}>Pending</option>
                            <option value="confirmed" {{ $smartRent->status == 'confirmed' ? 'selected' : '' }}>Confirmed</option>
                            <option value="completed" {{ $smartRent->status == 'completed' ? 'selected' : '' }}>Completed</option>
                            <option value="cancelled" {{ $smartRent->status == 'cancelled' ? 'selected' : '' }}>Cancelled</option>
                        </select>
                    </div>
                </div>
                @endif
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Sync dropdown selection to cards
            const armadaSelect = document.getElementById('armada-select');
            if (armadaSelect) {
                armadaSelect.addEventListener('change', function() {
                    if (this.value) selectArmada(this.value);
                });
            }
        });
        ...
        function selectArmada(armadaId) {
            ...
            if (selectedContainer) {
                ...
                if (radio) {
                    ...
                    const armadaSelectEl = document.getElementById('armada-select');
                    if (armadaSelectEl) {
                        armadaSelectEl.value = armadaId;
                    }
        ...
        function updateTotalHarga() {
            ...
            const hargaSopir = parseInt(selectedContainer.dataset.hargaSopir) || 0;
            ...
            // Hitung biaya layanan (dengan sopir) - use actual armada price from database
            let biayaLayanan = 0;
            const layananSelect = document.getElementById('layanan');
            
            if (layananSelect && layananSelect.value === 'dengan_sopir' && hargaSopir > 0) {
                biayaLayanan = hargaSopir * durasi * jumlahMobil;
            }

...
@section('content')
@php
    // Ambil semua data dari variabel yang dikirim controller
    
    // Data pesanan
    $order_number = $order_number ?? 'SR' . date('Ymd') . rand(100, 999);
    $vehicle_name = $vehicle_name ?? 'Toyota Hiace Commuter';
    $vehicle_price = $vehicle_price ?? 1200000;
    $driver_price_per_day = $driver_price_per_day ?? 0;
    $selected_driver_price_per_day = $selected_driver_price_per_day ?? $driver_price_per_day;
    $service = $service ?? 'with_driver';
    $duration = $duration ?? 1;
    $rent_date = $rent_date ?? date('Y-m-d');
    $vehicle_total = $vehicle_total ?? ($vehicle_price * $duration);
    $driver_total = $driver_total ?? 0;
    $total_price = $total_price ?? $vehicle_total;
    
    // Data customer
    $full_name = $customer_name ?? 'Haryantie Chinta Dewi';
    $phone = $customer_phone ?? '089653456787';
    $email = $customer_email ?? 'haryantiechintadewi@gmail.com';
    $pickup_location = $pickup_location ?? 'Jakarta - Kantor Pusat SmartRent';
    $payment_method = $payment_method ?? 'QRIS';
    $payment_time = $payment_time ?? date('d/m/Y H:i') . ' WIB';
    
    // Format tanggal
    $rent_date_formatted = $rent_date_formatted ?? date('d M Y', strtotime($rent_date));
    $rent_date_end_formatted = $rent_date_end_formatted ?? date('d M Y', strtotime($rent_date . ' + ' . ($duration - 1) . ' days'));
    $invoice_number = $invoice_number ?? 'INV/' . $order_number;
    
    // Hitung ulang untuk memastikan
    $calculated_vehicle_total = $vehicle_price * $duration;
    $calculated_driver_total = $selected_driver_price_per_day * $duration;
    $calculated_total = $calculated_vehicle_total + $calculated_driver_total;
    
    // Format untuk display
    $service_text = $service == 'with_driver' ? 'Dengan Sopir' : 'Lepas Kunci';
@endphp
    ...
    <div class="container-single">
        ...
        <div class="card">
            ...
            <div class="order-number-box">
                ...
                <div class="order-number-actions">
                    <a href="{{ route('smartrent.e-ticket', ['orderNumber' => $order_number]) }}" 
                       class="btn-eticket view" 
                       target="_blank">
                        <i class="fas fa-ticket-alt"></i> Lihat E-Ticket
                    </a>
                    <a href="{{ route('smartrent.e-ticket.download', ['orderNumber' => $order_number]) }}" 
            ...
            <div style="text-align: center; margin-bottom: 24px;">
                ...
                <div style="font-size: 16px; font-weight: 600; color: var(--dark);">{{ $invoice_number }}</div>
            </div>
            @endif
        </div>
        ...
        <div class="card">
            ...
            <div class="summary-grid">
                {{-- LEFT: VEHICLE & RENTAL --}}
                <div>
                    <h3 style="font-size: 14px; font-weight: 700; color: var(--dark); text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 16px; padding-bottom: 12px; border-bottom: 2px solid var(--primary-light); display: flex; align-items: center; gap: 8px;">
                    ...
                    <div class="summary-row">
                        ...
                        <span class="summary-value">{{ $service_text }}</span>
                    </div>
                    <div class="summary-row">
                        <span class="summary-label">Mulai Sewa</span>
                        <span class="summary-value">{{ $rent_date_formatted }}</span>
                    </div>
                    <div class="summary-row">
                        <span class="summary-label">Selesai Sewa</span>
                        <span class="summary-value">{{ $rent_date_end_formatted }}</span>
                    </div>
                    <div class="summary-row">
                    ...
                    <div class="summary-row">
                        ...
                        <span class="summary-value">{{ $pickup_location }}</span>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="summary-row">
                        ...
                        <span class="summary-value">{{ $full_name }}</span>
                    </div>
                    <div class="summary-row">
                        <span class="summary-label">Email</span>
                        <span class="summary-value" style="font-size: 12px;">{{ $email }}</span>
                    </div>
                    <div class="summary-row">
                    ...
                    <div class="summary-row">
                        ...
                        <span class="summary-value">{{ $payment_time }}</span>
                    </div>
                    <div class="summary-row">
            ...
            <div class="price-breakdown">
                <div class="price-item">
                    <span class="price-label">Harga Sewa Mobil / Hari</span>
                    <span class="price-value">Rp {{ number_format($vehicle_price, 0, ',', '.') }}</span>
                </div>
                
                <div class="price-item">
                    <span class="price-label">Durasi Sewa</span>
                    <span class="price-value">{{ $duration }} Hari</span>
                </div>
                
                <div class="price-item">
                    <span class="price-label">Total Sewa Mobil ({{ $duration }} hari)</span>
                    <span class="price-value highlight">Rp {{ number_format($vehicle_total, 0, ',', '.') }}</span>
                </div>
                
                @if($service == 'with_driver' && $selected_driver_price_per_day > 0)
                <div class="price-item">
                    <span class="price-label">Biaya Sopir / Hari</span>
                    <span class="price-value">Rp {{ number_format($selected_driver_price_per_day, 0, ',', '.') }}</span>
                </div>
                
                <div class="price-item">
                    <span class="price-label">Total Biaya Sopir ({{ $duration }} hari)</span>
                    <span class="price-value highlight">Rp {{ number_format($driver_total, 0, ',', '.') }}</span>
                </div>
                @endif
            </div>
            ...
            <div class="info-box">
                ...
                ✓ Bukti pembayaran telah dikirim ke email: <strong>{{ $email }}</strong><br>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        console.log('SmartRent payment success page loaded for order: {{ $order_number }}');
